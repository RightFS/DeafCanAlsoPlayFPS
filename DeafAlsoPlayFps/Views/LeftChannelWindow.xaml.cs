...
{
    public partial class LeftChannelWindow : Window
    {
        ...
        public LeftChannelWindow()
        {
            ...
            // 设置窗口位置到屏幕左侧
            SetWindowPositionLeft();

            Loaded += LeftChannelWindow_Loaded;
        }

        private void SetWindowPositionLeft()
        {
            try
            {
                // 获取主显示器尺寸
                ...
                Point? pos = SettingsHelper.Instance?.Settings?.LeftChannelPosition;
                ...
                _logger.Info($"左声道窗口位置设置为: ({this.Left}, {this.Top}) - 屏幕: {screenWidth}x{screenHeight}");
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "设置左声道窗口位置失败");
            }
        }

        private void LeftChannelWindow_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                _logger.Info("左声道窗口已加载");
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "加载左声道窗口失败");
            }
